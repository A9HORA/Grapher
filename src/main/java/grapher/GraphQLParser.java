...
public class GraphQLParser {
    ...
    public static List<ParsedOp> parseJsContent(String content) {
        ...
        while (tagMatcher.find()) {
            ...
            if (trimmed.length() > 15 && hasGqlKeyword) {
                ...
                if (queryVarName != null && queryVarName.length() >= 2) {
                    callSiteVars = extractCallSiteVariables(content, queryVarName, gqlBody);
                }
                // Fallback: try inline extraction from enclosing JSON.stringify
                if (callSiteVars == null) {
                    callSiteVars = extractInlineCallSiteVariables(content, matchPos, gqlBody);
                }
        ...
        while (rawMatcher.find()) {
            ...
            // Fallback: try inline extraction from enclosing JSON.stringify
            if (callSiteVars == null) {
                callSiteVars = extractInlineCallSiteVariables(content, rawMatcher.start(), cleaned);
            }
    ...
    public static List<ParsedOp> parseMinifiedJsContent(String content) {
        ...
        while (concatMatcher.find()) {
            ...
            if (cleaned.length() > 20) {
                String inlineVars = extractInlineCallSiteVariables(
                        content, concatMatcher.start(), cleaned);

                List<ParsedOp> parsed = parseQueryString(cleaned, null);
                if (inlineVars != null && !parsed.isEmpty()) {
                    List<ParsedOp> enriched = new ArrayList<>();
                    for (ParsedOp op : parsed) {
                        enriched.add(new ParsedOp(op.type, op.name, op.hash,
                                op.snippet, inlineVars));
                    }
                    results.addAll(enriched);
                } else {
                    results.addAll(parsed);
        ...
        while (propMatcher.find()) {
            ...
            if (!parsed.isEmpty()) {
                String callSiteVars = callSiteKey.length() >= 2
                        ? extractCallSiteVariables(content, callSiteKey, raw) : null;
                if (callSiteVars != null) {
                    List<ParsedOp> enriched = new ArrayList<>();
                    for (ParsedOp op : parsed) {
                        enriched.add(new ParsedOp(op.type, op.name, op.hash, op.snippet, callSiteVars));
        ...
        //    Also attempt inline call site extraction for JSON.stringify patterns.
        ...
        while (escapedMatcher.find()) {
            ...
            if (raw.length() > 15) {
                String inlineVars = extractInlineCallSiteVariables(
                        content, escapedMatcher.start(), raw);
        ...
        while (relayMatcher.find()) {
            ...
            String inlineVars = extractInlineCallSiteVariables(
                    content, relayMatcher.start(), raw);
            List<ParsedOp> parsed = parseQueryString(raw, null);
            if (inlineVars != null && !parsed.isEmpty()) {
                List<ParsedOp> enriched = new ArrayList<>();
                for (ParsedOp op : parsed) {
    ...
    static String extractCallSiteVariables(String content, String queryVarName,
        ...
        while (m.find()) {
            ...
            if (firstChar == '{' || firstChar == '[') {
                ...
                // Pre-normalize Terser idioms, then normalize JS values to JSON
                rawVars = preNormalizeJsValues(rawVars);
    ...
    /**
     * Extract variables from an inline {query: "...", variables: {...}} object
     * when the query is a string literal inside JSON.stringify(), not a variable.
     *
     * Forward-scans from each JSON.stringify( call in the file, extracts the
     * balanced object, checks if the query string match position falls within it,
     * and if both "query" and "variables" keys exist at depth 1 (dual-key gate).
     */
    static String extractInlineCallSiteVariables(String content, int queryStringPos,
                                                  String operationSignature) {
        String needle = "JSON.stringify(";
        int searchFrom = 0;

        while (searchFrom < content.length()) {
            int sPos = content.indexOf(needle, searchFrom);
            if (sPos < 0) break;
            searchFrom = sPos + needle.length();

            int objStart = sPos + needle.length();
            while (objStart < content.length() && Character.isWhitespace(content.charAt(objStart)))
                objStart++;
            if (objStart >= content.length() || content.charAt(objStart) != '{') continue;

            String region = content.substring(objStart);
            String fullObj = extractBalancedBlock(region, 0);
            if (fullObj == null) continue;

            int objEnd = objStart + fullObj.length();
            if (queryStringPos < objStart || queryStringPos >= objEnd) continue;

            int queryKeyPos = findKeyInObject(fullObj, "query");
            if (queryKeyPos < 0) continue;
            int varKeyPos = findKeyInObject(fullObj, "variables");
            if (varKeyPos < 0) continue;

            int colonPos = fullObj.indexOf(':', varKeyPos);
            if (colonPos < 0) continue;
    ...
    /**
     * Pre-normalize Terser/minifier idioms in a raw JS variables string
     * BEFORE normalizeJsObjectToJson processes it. This keeps the character-
     * walking normalizer untouched.
     *
     * Handles three specific patterns:
     * 1. void 0 === x ? "literal" : x  -> "literal"  (Terser default-value idiom)
     * 2. Number(expr)                   -> 0           (numeric coercion)
     * 3. (0,fn.X)(args)                -> null         (Webpack indirected call)
     */
    private static final Pattern TERSER_DEFAULT_VALUE = Pattern.compile(
            "(?:void\\s+0|undefined)\\s*===?\\s*[a-zA-Z_$]\\w*\\s*\\?\\s*(\"[^\"]*\")\\s*:\\s*[a-zA-Z_$]\\w*");

    private static final Pattern JS_NUMBER_CALL = Pattern.compile(
            "Number\\s*\\([^)]*\\)");

    // Bounded [^)]{0,200} prevents dangling-paren corruption on nested parens.
    private static final Pattern WEBPACK_INDIRECT_CALL = Pattern.compile(
            "\\(0\\s*,\\s*[a-zA-Z_$][\\w.]*\\)\\s*\\([^)]{0,200}\\)");

    static String preNormalizeJsValues(String js) {
        if (js == null) return js;
        js = TERSER_DEFAULT_VALUE.matcher(js).replaceAll("$1");
        js = JS_NUMBER_CALL.matcher(js).replaceAll("0");
        js = WEBPACK_INDIRECT_CALL.matcher(js).replaceAll("null");
        return js;
    }
    ...
    private static String normalizeJsObjectToJson(String js, String operationSignature) {
        ...
        StringBuilder out = new StringBuilder();
        String lastKey = null; // tracks the most recent unquoted key for value fallback
        int i = 0;
        int len = js.length();
        ...
        while (i < len) {
            ...
            if (Character.isLetter(c) || c == '_' || c == '$') {
                ...
                // Consume property access chains: e.productType, e.metaData.foo, etc.
                // Without this, "e.productType" is split into "e" and "productType",
                // each resolved separately, producing doubled output like """""".
                ...
                if (j < len && js.charAt(j) == ':') {
                    // Unquoted key — quote it and remember for value fallback.
                    // For dotted keys (rare), use only the last segment.
                    String keyName = ident.contains(".")
                            ? ident.substring(ident.lastIndexOf('.') + 1)
                            : ident;
                    out.append('"').append(keyName).append('"');
                    lastKey = keyName;
                } else {
                    // Unresolvable value reference (e.g., e.productType, t, someVar).
                    // First try the JS variable name in the type map,
                    // then try the most recent key name (the GraphQL variable name).
                    String lookupName = ident.contains(".")
                            ? ident.substring(ident.lastIndexOf('.') + 1)
                            : ident;
                    String placeholder = typeMap.get(lookupName.toLowerCase());
                    if (placeholder == null && lastKey != null) {
                        placeholder = typeMap.get(lastKey.toLowerCase());
                    }
                    if (placeholder == null) {
                        placeholder = "null";
                    }
    ...
    private static Map<String, String> buildOperationTypeMap(String operationSignature) {
        ...
        while (m.find()) {
            ...
            switch (base) {
                ...
                default:
                    if (looksLikeEnumTypeStatic(base)) {
                        placeholder = "\"\"";
                    } else if (looksLikeInputObjectTypeStatic(base)) {
                        placeholder = "{}";
                    } else {
                        placeholder = "null";
                    }
    ...
    private static boolean looksLikeEnumTypeStatic(String typeName) {
        String[] enumSuffixes = {
    ...
    private static boolean looksLikeInputObjectTypeStatic(String typeName) {
