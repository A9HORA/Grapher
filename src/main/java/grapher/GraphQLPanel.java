    ...
    private HttpRequest resolveRequest(GraphQLEntry entry) {
        ...
        if (entry.hasSendableRequest()) {
            ...
            if (mergedBody != null && !mergedBody.equals(entry.operationBody())) {
                // Build a new body with the merged query
                String originalBody = original.bodyToString();
                try {
                    // Replace the query field in the JSON body
                    String escapedMerged = mergedBody.replace("\\", "\\\\")
                            .replace("\"", "\\\"")
                            .replace("\n", "\\n")
                            .replace("\t", "\\t");

                    // Find and replace the "query":"..." value
                    int qStart = originalBody.indexOf("\"query\"");
                    if (qStart >= 0) {
                        int colonPos = originalBody.indexOf(':', qStart + 7);
                        if (colonPos >= 0) {
                            int valStart = originalBody.indexOf('"', colonPos + 1);
                            if (valStart >= 0) {
                                // Find the end of the query string value
