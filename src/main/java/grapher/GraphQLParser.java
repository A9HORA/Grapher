...
public class GraphQLParser {
    ...
    public static List<ParsedOp> parseJsContent(String content) {
        ...
        // First, build a variable map for resolving ${varName} interpolations
        Map<String, String> templateVarMap = buildStringVarMap(content);

        Matcher tagMatcher = JS_GQL_TAG.matcher(content);
        while (tagMatcher.find()) {
            String rawBody = tagMatcher.group(1);
            // Resolve ${varName} interpolations using the variable map
            String resolved = resolveTemplateInterpolations(rawBody, templateVarMap);
    ...
    public static List<ParsedOp> parseMinifiedJsContent(String content) {
        ...
        while (concatMatcher.find()) {
            ...
            while (pos < content.length()) {
                ...
                fullQuery += resolveConcatArgPositioned(concatArg, concatPos, posVarMap, content);
            }
     ...
     * Substitutes each ${varName} with the variable's string value from the map.
     * Unknown variables are removed (replaced with empty string).
     */
