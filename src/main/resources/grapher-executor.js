...
function resolveVariables(source) {
    ...
    // Phase 3: Resolve template literal interpolations
    // Matches tagged templates like: kr.J1`mutation foo(${vv} ${uv}) { ${gv} ... }`
    // where vv, uv, gv are string variables in the varMap
    const templateRegex = /[a-zA-Z_$][a-zA-Z0-9_$]*(?:\.[a-zA-Z_$][a-zA-Z0-9_$]*)?\s*`((?:[^`])*)`/g;
    while ((m = templateRegex.exec(source)) !== null) {
        let body = m[1];
        if (!body || body.length < 10) continue;
        // Check if it contains ${...} interpolations
        if (!body.includes('${')) {
            // No interpolations — check if it's a direct GraphQL string
            const cleaned = body.replace(/\s+/g, ' ').trim();
            if (isGraphQLQuery(cleaned) && cleaned.length > 20) {
                const nameMatch = cleaned.match(/^(query|mutation|subscription)\s+(\w+)/);
                recordCapture(cleaned, nameMatch ? nameMatch[2] : '', {});
            }
            continue;
        }
        // Resolve ${varName} interpolations
        let resolved = body.replace(/\$\{([a-zA-Z_$][a-zA-Z0-9_$]*)\}/g, (match, varName) => {
            return varMap.has(varName) ? varMap.get(varName) : '';
        });
    ...
    // Phase 4: Capture standalone GraphQL strings from the variable map
