...
public class GrapherExtension implements BurpExtension, ExtensionUnloadingHandler {
    ...
    /** Tracks the JS executor thread for clean shutdown. */
    private volatile Thread jsExecutorThread;
    ...
    public void extensionUnloaded() {
        ...
        // Interrupt the JS executor thread if it's running
        Thread execThread = jsExecutorThread;
    ...
    private class GqlHttpHandler implements HttpHandler {
        ...
        public ResponseReceivedAction handleHttpResponseReceived(HttpResponseReceived response) {
            try {
                ...
                // Detect PersistedQueryNotFound in response — tags the operation
                // so the tester knows it was previously persisted-only
                try {
                    String respBody = response.bodyToString();
                    if (respBody != null && (respBody.contains("PERSISTED_QUERY_NOT_FOUND") ||
                                              respBody.contains("PersistedQueryNotFound"))) {
                        // Extract the operation name from the initiating request
                        String reqBody = initiatingRequest.bodyToString();
                        if (reqBody != null) {
                            java.util.regex.Matcher opMatcher = java.util.regex.Pattern.compile(
                                    "\"operationName\"\\s*:\\s*\"([^\"]+)\"").matcher(reqBody);
                            if (opMatcher.find()) {
                                String opName = opMatcher.group(1);
                                tableModel.tagEntriesByOpName(opName, "persisted-retry");
                                logging.logToOutput("[!] PersistedQueryNotFound for: " + opName +
                                        " — server requires persisted queries for this operation");
                            }
                        }
                    }
                } catch (Exception ex) {
                    // Non-critical — don't let detection failure break the handler
                }
                ...
                if (jsParserPool != null && !jsParserPool.isShutdown()) {
                    jsParserPool.submit(() -> {
                        try {
                            ...
                            // Catch Throwable (not just Exception) to survive
                            // StackOverflowError from regex backtracking on huge JS
                            logging.logToError("JS parse error: " + ex.getClass().getSimpleName() + ": " + ex.getMessage());
                        }
                    });
                }
    ...
    private void processRequestFromResponse(HttpRequest request, HttpRequestResponse reqResp) {
        ...
        if ("POST".equalsIgnoreCase(method)) {
            ...
            if (body != null && !body.isEmpty() && looksLikeGraphQLBody(body)) {
                ...
                // Extract the "variables" JSON value from the HTTP POST body.
                // This captures real runtime values (cguid, authToken, etc.)
                // that the browser actually sent to the server.
                String httpVariablesJson = extractHttpVariables(body);

                List<GraphQLParser.ParsedOp> ops = GraphQLParser.parseJsonBody(body);
                for (GraphQLParser.ParsedOp op : ops) {
                    addFinding(url, path, method,
                            op.hash != null ? GraphQLEntry.Source.PERSISTED_QUERY : GraphQLEntry.Source.HTTP_POST,
                            op.type, op.name, op.hash, op.snippet, reqResp, httpVariablesJson);
                }
            }
        }
    ...
    private void processJsResponse(HttpResponseReceived response, HttpRequestResponse reqResp) {
        ...
        // Log warning if file exceeds the parser's size limit
        if (body.length() > 15 * 1024 * 1024) {
            logging.logToOutput("[!] Skipped oversized JS file (" +
                    (body.length() / 1024 / 1024) + "MB): " +
        ...
        for (GraphQLParser.ParsedOp op : ops) {
            addFinding(url, path, "GET", GraphQLEntry.Source.JS_FILE,
                    op.type, op.name, op.hash, op.snippet, reqResp, op.extractedVariables);
        }
    ...
    public HttpRequest buildGqlRequest(String operationBody, String operationName,
        ...
        return buildGqlRequest(operationBody, operationName, operationType, persistedHash, null);
    }

    public HttpRequest buildGqlRequest(String operationBody, String operationName,
                                       String operationType, String persistedHash,
                                       String extractedVariablesJson) {
        ...
        if ("doc_id".equals(operationType) && persistedHash != null && !persistedHash.isEmpty()) {
            ...
            String escapedBody = escapeForJson(operationBody);

            // Use call site extracted variables if available, otherwise fall back
            // to type-inference-based placeholders from the operation signature
            String variablesJson = (extractedVariablesJson != null && !extractedVariablesJson.isEmpty())
                    ? extractedVariablesJson
                    : buildVariablePlaceholders(operationBody);
    ...
    private String buildVariablePlaceholders(String operationBody) {
        ...
        // Variables can be comma-separated OR space-separated (both are valid GraphQL).
        // The type capture stops at the next $, comma, or closing paren.
        // Handles: $id: ID!, $name: String, $input: [CreateInput!]!
    ...
    String getPlaceholderForType(String type) {
        ...
        switch (base) {
            ...
            case "ID":
                return "\"\"";
            default:
                // Heuristic: types with common enum suffixes are likely enums (string values)
                // rather than input objects. Without schema knowledge we can't distinguish them,
                // but this covers the most common naming conventions.
                if (looksLikeEnumType(base)) {
                    return "\"\"";
                }
                // Heuristic: types with common input object suffixes get {}
                // instead of null. The server error for {} names the required
                // inner fields — more actionable than a non-null error for null.
                if (looksLikeInputObjectType(base)) {
                    return "{}";
                }
                // Unrecognized types get null. The server either accepts it (nullable)
                // or returns a clean error with the exact variable name, type, and
                // non-null constraint — actionable information for the tester.
                return "null";
        }
    }

    /**
     * Heuristic check for enum type names based on common naming conventions.
     * Types ending with these suffixes are likely enums that take string values
     * rather than input objects that take nested fields.
     */
    private boolean looksLikeEnumType(String typeName) {
        String[] enumSuffixes = {
            "Method", "Reason", "Type", "Status", "Kind", "Mode", "Format",
            "Direction", "Level", "Role", "State", "Category", "Sort", "Order",
            "Enum", "Priority", "Frequency", "Unit", "Currency", "Locale",
            "Channel", "Platform", "Visibility", "Permission", "Action",
            "Phase", "Stage", "Severity", "Rating", "Grade", "Tier",
            "By", "Strategy", "Algorithm", "Policy"
        };
        for (String suffix : enumSuffixes) {
            if (typeName.endsWith(suffix)) return true;
        }
        // Also match types that are ALL_CAPS (common enum convention)
        if (typeName.matches("[A-Z][A-Z_0-9]+")) return true;
        return false;
    }

    /**
     * Heuristic check for input object type names based on common naming conventions.
     * Types ending with these suffixes are almost always input objects that take
     * nested fields rather than scalar or enum values.
     *
     * Must be called AFTER looksLikeEnumType — enum check takes priority.
     * Only runs on GraphQL type names from operation signatures, never on JS
     * variable names, so suffixes like "Data" are safe.
     */
    private boolean looksLikeInputObjectType(String typeName) {
        String[] inputSuffixes = {
            "Input", "Request", "Payload", "Param", "Params", "Options",
            "Config", "Args", "Filter", "Where", "Criteria",
            "Data", "Settings", "Preferences", "Spec", "Dto"
        };
        for (String suffix : inputSuffixes) {
            if (typeName.endsWith(suffix)) return true;
        }
        return false;
    }
    ...
    private String buildRawRequest(HttpRequest template, String jsonBody, String operationName) {
        ...
        raw.append("Content-Length: ").append(jsonBody.getBytes(java.nio.charset.StandardCharsets.UTF_8).length).append("\r\n");
        raw.append("\r\n");
        raw.append(jsonBody);
    ...
    /** Called by GraphQLPanel to register the executor thread for clean shutdown. */
    public void setJsExecutorThread(Thread t) {
        this.jsExecutorThread = t;
    }
    ...
    /**
     * Extract the "variables" JSON value from an HTTP POST body.
     * Returns the raw JSON object/array string, or null if not found.
     *
     * The HTTP POST body is well-formed JSON (unlike minified JS), so this
     * uses simple depth-tracked extraction rather than the full normalizer.
     *
     * Only extracts from single-operation requests. Batched requests
     * ([{...},{...}]) are deferred — each batch item would need its
     * variables matched to the correct operation by operationName.
     *
     * Called from processRequestFromResponse only — NOT from
     * handleHttpRequestToBeSent (avoids the Double Work anti-pattern).
     */
    private String extractHttpVariables(String body) {
        if (body == null || body.isEmpty()) return null;

        // Skip batched requests — single operation only
        String trimmed = body.trim();
        if (trimmed.startsWith("[")) return null;

        // Find "variables" key at depth 1 in the JSON body
        ...
        for (int i = 0; i < trimmed.length(); i++) {
            ...
            if (c == '"') {
                if (depth == 1 && i + target.length() <= trimmed.length() &&
                    trimmed.substring(i, i + target.length()).equals(target)) {
                    varKeyPos = i;
                    break;
                }
                inString = true;
        ...
        if (varKeyPos < 0) return null;

        int colonPos = trimmed.indexOf(':', varKeyPos + target.length());
        ...
        if (firstChar == '{' || firstChar == '[') {
            int extractDepth = 0;
            boolean extractInString = false;
            int end = valueStart;

            while (end < trimmed.length()) {
                char ec = trimmed.charAt(end);
                if (extractInString) {
                    if (ec == '\\' && end + 1 < trimmed.length()) { end += 2; continue; }
                    if (ec == '"') extractInString = false;
                    end++;
                    continue;
                }
                if (ec == '"') { extractInString = true; end++; continue; }
                if (ec == '{' || ec == '[') extractDepth++;
                else if (ec == '}' || ec == ']') {
                    extractDepth--;
                    if (extractDepth == 0) {
                        end++;
            ...
            String result = trimmed.substring(valueStart, end);
            if ((result.startsWith("{") && result.endsWith("}")) ||
                (result.startsWith("[") && result.endsWith("]"))) {
                // Skip empty objects — no value in storing {}
                if (result.equals("{}") || result.equals("[]")) return null;
                return result;
            }
        }
    ...
    private void addFinding(String url, String endpoint, String method,
        ...
        addFinding(url, endpoint, method, source, opType, opName, hash, snippet, reqResp, null);
    }

    private void addFinding(String url, String endpoint, String method,
                            GraphQLEntry.Source source, String opType,
                            String opName, String hash, String snippet,
                            HttpRequestResponse reqResp, String extractedVariablesJson) {

        // Synchronized dedup check + update
        if (tableModel.isDuplicate(endpoint, opName, opType, hash)) {
            // Try to upgrade an existing entry with better reqResp or longer body
            tableModel.updateRequestResponse(endpoint, opName, opType, reqResp, snippet, extractedVariablesJson);
            return;
        }
        ...
        GraphQLEntry entry = new GraphQLEntry.Builder()
                ...
                .extractedVariablesJson(extractedVariablesJson)
                .build();
        ...
        logging.logToOutput(String.format("[GQL] %s %s | %s %s | %s%s",
                source.label(), endpoint, opType, opName,
                hash != null ? "hash=" + hash : "",
                extractedVariablesJson != null ? " [vars extracted]" : ""));
    }
}
